...
控制反转——在传统的程序设计中，当调用者需要被调用者的协助时，通常由调用者来创建被调用者的实例。当应用了IoC，一个对象依赖的其它对象会通过被动的方式传递进来，而不是这个对象自己创建或者查找依赖对象。

面向切面——Spring提供了[面向切面编程](https://baike.baidu.com/item/%E9%9D%A2%E5%90%91%E5%88%87%E9%9D%A2%E7%BC%96%E7%A8%8B)的丰富支持，允许通过分离应用的业务逻辑与系统级服务（例如审计（auditing）和事务（transaction）管理）进行内聚性的开发。
...
FactoryBean也是接口，主要用于返回BeanFactory生产好的bean对象，并且实现了简单工厂模式和装饰模式，如果要获取FactoryBean对象，请在id前面加一个&符号来获取。
...
依赖注入，是IOC的一个方面。你不在代码里直接创建对象，但是要在配置文件里描述对象如何被创建和组装，之后一个容器（IOC容器）负责把他们组装起来。

#### 有哪些不同类型的依赖注入方式？

•	构造器依赖注入：构造器依赖注入通过容器触发一个类的构造器来实现的，该方法有一系列参数，每个参数代表一个对其他类的依赖。
